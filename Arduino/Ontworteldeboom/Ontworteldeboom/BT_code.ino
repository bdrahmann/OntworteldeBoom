...
void ReactieOpk(String prefinfo) {	// lees de Arduino settings uit. Meerdere settings in info. En sla op in EPROM
	...
	// drooglevel1
	e = prefinfo.indexOf ("$");
	String strDrooglevel1 = prefinfo.substring(b,e);
	Drooglevel1 = strDrooglevel1.toInt();
	SchrijfEprom(11, 14, strDrooglevel1);
	...
	String strDrooglevel2 = prefinfo.substring(b,e);
	Drooglevel2 = strDrooglevel2.toInt();
	SchrijfEprom(15, 18, strDrooglevel2);
	...
	b = e + 1;	// het nieuwe begin
	e = prefinfo.indexOf ("$",b);
	String strDrooglevel3 = prefinfo.substring(b,e);
	Drooglevel3 = strDrooglevel3.toInt();
	SchrijfEprom(19, 22, strDrooglevel3);
	...
	int Droogtijdsec = strDroogtijdsec.toInt();	
	SchrijfEprom(23, 26, strDroogtijdsec);
	Serial.print("droogtijdsec: ");
	Serial.println(Droogtijdsec);
	
	// druppelspeling
	b = e + 1;	// het nieuwe begin
	e = prefinfo.indexOf ("$",b);
	String strDruppelspeling = prefinfo.substring(b,e);
	Druppelspeling = strDruppelspeling.toInt();
	SchrijfEprom(27, 30, strDruppelspeling);
	...
	b = e + 1;	// het nieuwe begin
	e = prefinfo.indexOf ("$",b);
	String strSamples = prefinfo.substring(b,e);
	//  TODO  uitzoeken of het volgende  wel kan
	int ra_hulp = strSamples.toInt();  // als het kan de // weghalen
	SchrijfEprom(31, 34, strSamples);
	Serial.print("samples_hulp: ");
	Serial.println(ra_hulp);
	
	// laagwaterdelay
	b = e + 1;	// het nieuwe begin
	e = prefinfo.indexOf ("$",b);
	String strLaagwaterdelaysec = prefinfo.substring(b,e);
	int laagwater_delaysec = strLaagwaterdelaysec.toInt();	
	SchrijfEprom(35, 38, strLaagwaterdelaysec);
	Serial.print("vlotterdelaysec: ");
	Serial.println(laagwater_delaysec);
